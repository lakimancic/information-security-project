use crate::crypto::block::BlockCipher;
use crate::crypto::errors::CryptoError;

const BLOCK_SIZE: usize = 16;
const NR: usize = 14;

const SBOX: [u8; 256] = [
    ,
];

const INV_SBOX: [u8; 256] = [
    ,
];

const RCON: [u8; 15] = [
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36,0x6C,0xD8,0xAB,0x4D
];


pub struct AES256 {
    round_keys: [u8; 240],
}

impl AES256 {
    pub fn new(key: Vec<u8>) -> Result<Self, CryptoError> {
        let key: [u8; 32] = key.try_into()
            .map_err(|_| CryptoError::InvalidKeyLength)?;

        let mut round_keys = [0u8; 240];
        round_keys[..32].copy_from_slice(&key);

        let mut i = 32;
        let mut rcon_i = 1;

        while i < 240 {
            let mut temp = [
                round_keys[i - 4],
                round_keys[i - 3],
                round_keys[i - 2],
                round_keys[i - 1],
            ];

            if i % 32 == 0 {
                temp.rotate_left(1);
                for b in &mut temp {
                    *b = SBOX[*b as usize];
                }
                temp[0] ^= RCON[rcon_i];
                rcon_i += 1;
            } else if i % 32 == 16 {
                for b in &mut temp {
                    *b = SBOX[*b as usize];
                }
            }

            for j in 0..4 {
                round_keys[i] = round_keys[i - 32] ^ temp[j];
                i += 1;
            }
        }

        Ok(Self { round_keys })
    }

    fn add_round_key(state: &mut [u8; 16], rk: &[u8]) {
        for i in 0..16 {
            state[i] ^= rk[i];
        }
    }

    fn sub_bytes(state: &mut [u8; 16]) {
        for b in state.iter_mut() {
            *b = SBOX[*b as usize];
        }
    }

    fn shift_rows(state: &mut [u8; 16]) {
        let tmp = state.clone();
        state[1]  = tmp[5];  state[5]  = tmp[9];  state[9]  = tmp[13]; state[13] = tmp[1];
        state[2]  = tmp[10]; state[6]  = tmp[14]; state[10] = tmp[2];  state[14] = tmp[6];
        state[3]  = tmp[15]; state[7]  = tmp[3];  state[11] = tmp[7];  state[15] = tmp[11];
    }

    fn xtime(x: u8) -> u8 {
        (x << 1) ^ if x & 0x80 != 0 { 0x1b } else { 0 }
    }

    fn mix_columns(state: &mut [u8; 16]) {
        for c in 0..4 {
            let i = c * 4;
            let a = state[i];
            let b = state[i + 1];
            let c_ = state[i + 2];
            let d = state[i + 3];

            let t = a ^ b ^ c_ ^ d;
            state[i]     ^= t ^ AES256::xtime(a ^ b);
            state[i + 1] ^= t ^ AES256::xtime(b ^ c_);
            state[i + 2] ^= t ^ AES256::xtime(c_ ^ d);
            state[i + 3] ^= t ^ AES256::xtime(d ^ a);
        }
    }

    fn inv_sub_bytes(state: &mut [u8; 16]) {
        for b in state.iter_mut() {
            *b = INV_SBOX[*b as usize];
        }
    }

    fn inv_shift_rows(state: &mut [u8; 16]) {
        let tmp = state.clone();
        state[1]  = tmp[13]; state[5]  = tmp[1];  state[9]  = tmp[5];  state[13] = tmp[9];
        state[2]  = tmp[10]; state[6]  = tmp[14]; state[10] = tmp[2];  state[14] = tmp[6];
        state[3]  = tmp[7];  state[7]  = tmp[11]; state[11] = tmp[15]; state[15] = tmp[3];
    }

    fn mul(mut a: u8, mut b: u8) -> u8 {
        let mut r = 0;
        while b != 0 {
            if b & 1 != 0 {
                r ^= a;
            }
            a = AES256::xtime(a);
            b >>= 1;
        }
        r
    }

    fn inv_mix_columns(state: &mut [u8; 16]) {
        for c in 0..4 {
            let i = c * 4;
            let a = state[i];
            let b = state[i + 1];
            let c_ = state[i + 2];
            let d = state[i + 3];

            state[i]     = AES256::mul(a,14) ^ AES256::mul(b,11) ^ AES256::mul(c_,13) ^ AES256::mul(d,9);
            state[i + 1] = AES256::mul(a,9)  ^ AES256::mul(b,14) ^ AES256::mul(c_,11) ^ AES256::mul(d,13);
            state[i + 2] = AES256::mul(a,13) ^ AES256::mul(b,9)  ^ AES256::mul(c_,14) ^ AES256::mul(d,11);
            state[i + 3] = AES256::mul(a,11) ^ AES256::mul(b,13) ^ AES256::mul(c_,9)  ^ AES256::mul(d,14);
        }
    }
}

impl BlockCipher for AES256 {
    fn block_size(&self) -> usize { BLOCK_SIZE }

    fn encrypt_block(&self, block: &mut [u8]) -> Result<(), CryptoError> {
        if block.len() != 16 {
            return Err(CryptoError::InvalidBlockSize);
        }

        let mut state = [0u8; 16];
        state.copy_from_slice(block);

        AES256::add_round_key(&mut state, &self.round_keys[0..16]);

        for round in 1..NR {
            AES256::sub_bytes(&mut state);
            AES256::shift_rows(&mut state);
            AES256::mix_columns(&mut state);
            AES256::add_round_key(&mut state, &self.round_keys[round * 16..(round + 1) * 16]);
        }

        AES256::sub_bytes(&mut state);
        AES256::shift_rows(&mut state);
        AES256::add_round_key(&mut state, &self.round_keys[NR * 16..(NR + 1) * 16]);

        block.copy_from_slice(&state);
        Ok(())
    }

    fn decrypt_block(&self, block: &mut [u8]) -> Result<(), CryptoError> {
        if block.len() != 16 {
            return Err(CryptoError::InvalidBlockSize);
        }

        let mut state = [0u8; 16];
        state.copy_from_slice(block);

        AES256::add_round_key(&mut state, &self.round_keys[NR * 16..(NR + 1) * 16]);

        for round in (1..NR).rev() {
            AES256::inv_shift_rows(&mut state);
            AES256::inv_sub_bytes(&mut state);
            AES256::add_round_key(&mut state, &self.round_keys[round * 16..(round + 1) * 16]);
            AES256::inv_mix_columns(&mut state);
        }

        AES256::inv_shift_rows(&mut state);
        AES256::inv_sub_bytes(&mut state);
        AES256::add_round_key(&mut state, &self.round_keys[0..16]);

        block.copy_from_slice(&state);
        Ok(())
    }
}
